...
function setupUI() {
    ...
    const createWebPopover = (key, url, title, buttonEl, isRightAligned) => {
        ...
        popover.innerHTML = `
            <div class="git-view-header" style="height:44px; min-height:44px; border-bottom: none; display:flex; align-items:center; justify-content:space-between; padding:0 12px; background: #14151c; user-select: none; cursor: move;">
                <div style="display:flex; align-items:center; gap:10px; flex: 1; overflow: hidden; margin-right: 12px;">
                    ...
                    <span class="git-wv-settings" style="cursor: pointer; color: var(--text-muted); display: flex; align-items: center; justify-content: center; flex-shrink: 0; opacity: 0.8; transition: color 0.2s, opacity 0.2s;" title="Settings">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="pointer-events: none;">
        ...
        const popoverSettingsBtn = popover.querySelector('.git-wv-settings');
        if (popoverSettingsBtn) {
            popoverSettingsBtn.onclick = (e) => {
                e.stopPropagation();
                if (typeof window.openLocalSettingsModal === 'function') {
                    window.openLocalSettingsModal();
                } else {
                    const localSettingsBtn = document.getElementById('btn-local-settings');
                    if (localSettingsBtn) localSettingsBtn.click();
                }
            };
        }
    ...
    const settingsAgentBtn = document.getElementById('settings-agent');
    if (settingsAgentBtn) {
        settingsAgentBtn.onclick = (e) => {
            e.stopPropagation();
            window.openLocalSettingsModal();
        };
    }
    ...
    const updateAgentBadge = () => {
        ...
        if (wv && wv.src && !wv.src.startsWith('about:blank')) {
            ...
            if (badge) badge.innerText = `GRAVITY`; if (headerIcon) headerIcon.src = 'assets/icon.png'; if (chatIn) chatIn.placeholder = `Ask AI...`;
        }
    };
