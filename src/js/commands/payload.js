...
async function injectWebPayload(webPayload, fileCount = 0, currentFileIndex = 0, isAppend = false, clickSend = true) {
    ...
    return new Promise((resolve, reject) => {
                ...
                try {
                    ...
                    if (inputEl.tagName === 'TEXTAREA' || inputEl.tagName === 'INPUT') {
                        ...
                        try {
                            ...
                            dataTransfer.setData('text/plain', decodedPayload);
                            ...
                            injected = (inputEl.value || inputEl.innerText || "").trim().length > 0;
                        } catch (e) {}
                        ...
                        if (!injected || (inputEl.value || inputEl.innerText || "").trim() === "") {
                            try {
        ...
        wv.executeJavaScript(injectionScript).then(async (status) => {
            ...
            // 3단계: 짧은 대기 후 전송 버튼 클릭 (100ms로 단축하여 포커스 누수 방지 및 초고속화)
