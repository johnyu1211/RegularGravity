        ...
        wv.addEventListener('console-message', (e) => {
            ...
            if (e.message.startsWith('[GUEST_INPUT_HEIGHT]:')) {
                ...
                if (!isNaN(h)) {
                    ...
                    // Height increase fallback: if chat input grows during drag-drop mode, it confirms successful attachment
                    const isHeightIncreased = (prevH > 0) ? (h > prevH) : (h > 120);
                    if (window.dragDropMode && isHeightIncreased) {
                        ...
                        if (uploadingItem) {
                            ...
                            setTimeout(() => {
                                ...
                                if (currentH < 80) {
                                    ...
                                    // 1. Rollback item status to PENDING on host
                                    ...
                                    // 3. Dispatch rollback command to guest webview context using IPC
                                    if (wv && typeof wv.send === 'function') {
                                        wv.send('rollback-file-status', targetPath);
                                    }
                                } else {
