...
ipcMain.handle('vault-get-tree', async (event, projectPath) => {
    ...
    const ignore = ['node_modules', '.git', 'gravity_vault', 'dist', 'build', 'lib', 'scratch', 'out', '.vs', '.idea', 'data'];
    const results = [];
...
ipcMain.handle('vault-search', async (event, { query }) => {
    ...
    const vaultPath = path.join(process.cwd(), 'data', 'gravity_vault', getProjectHash());
    const results = [];
...
ipcMain.on('ondragstart', (event, filePath) => {
    ...
    const iconPath = path.join(__dirname, 'assets/png.png');
...
ipcMain.on('confirm-completed-item', (event, absPath) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('confirm-completed-item', absPath);
    }
});

ipcMain.on('rollback-completed-item', (event, absPath) => {
    if (mainWindow && !mainWindow.isDestroyed()) {
        mainWindow.webContents.send('rollback-completed-item', absPath);
    }
});
